...
def create_random_example(
    ...
    values = dict(
        input_ids=np.random.randint(vocab_size, size=[max_seq_length], dtype=np.int32),
        ...
        row_ids=np.random.randint(num_rows, size=[max_seq_length], dtype=np.int32),
        ...
        numeric_relations=np.random.randint(10, size=[max_seq_length], dtype=np.int32),
    )
    if task_type is table_dataset.TableTask.PRETRAINING:
        ...
        values["masked_lm_weights"] = np.random.rand(max_predictions_per_seq).astype(
            "f"
        )
        values["next_sentence_labels"] = np.random.randint(2, size=[1], dtype=np.int32)
    elif task_type in (table_dataset.TableTask.CLASSIFICATION,):
        ...
        if add_answer:
            answer = (
                ...
                else np.random.uniform(-100, 100, size=[1])
            )
            values["answer"] = answer.astype(np.float32)
            values["numeric_values_scale"] = np.random.randint(
                low=1, high=5, size=[max_seq_length], dtype=np.int32
            ).astype(np.float32)
            numeric_values = np.where(
                np.random.rand(max_seq_length) < 0.5,
                np.nan,
    ...
    if add_candidate_answers:
        ...
        for size in values["can_sizes"]:
            indices.append(
                np.random.choice(max_seq_length, size=[size], replace=False).astype(
                    np.int32
                )
            )
    ...
    if include_id:
        question_id = "".join(random.choice(string.ascii_lowercase) for i in range(10))
...
def create_random_dataset(num_examples, batch_size, repeat, generator_kwargs):
    ...
    parse_fn = table_dataset.parse_table_examples(
        ...
        add_aggregation_function_id=generator_kwargs["add_aggregation_function_id"],

...
flags.DEFINE_integer(
    "down_projection_dim",
    0,
    "Representation dimension of the query/table"
...
flags.DEFINE_bool(
    ...
    True,
    "If true, expects to load"
    "a checkpoint of a single encoder, that would be used to"
    "initialize both encoders.",
)

flags.DEFINE_integer("max_query_length", 128, "The query is capped to this length.")
...
flags.DEFINE_bool(
    "use_out_of_core_negatives",
    False,
    "If true, use all the negatives when" "using many TPU cores.",
)
...
flags.DEFINE_bool(
    "use_mined_negatives",
    False,
    "If true, use mined negatives that should be given as" "additional table features.",
)
...
def _predict_and_export_metrics(
    ...
    # Compute precision@k.
    if not FLAGS.evaluated_checkpoint_step or not FLAGS.evaluated_checkpoint_metric:
    ...
    if FLAGS.do_eval:
        ...
        for _, checkpoint in experiment_utils.iterate_checkpoints(
            ...
            marker_file_prefix=os.path.join(
                estimator.model_dir, f"eval_{FLAGS.eval_name}"
            ),

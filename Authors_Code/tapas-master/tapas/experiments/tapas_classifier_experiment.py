...
flags.DEFINE_integer("max_num_columns", 32, "Maximum number of columns in a table.")

flags.DEFINE_integer("max_num_rows", 64, "Maximum number of rows in a table.")

flags.DEFINE_bool(
    "average_logits_per_cell", False, "Whether to average logits in the model."
)

flags.DEFINE_integer(
    "minutes_to_sleep_before_predictions",
    5,
...
flags.DEFINE_float(
    "aggregation_loss_importance",
    1.0,
    "Weight that determines the aggregation"
...
flags.DEFINE_bool(
    ...
    True,
    "Whether to consider the answer as"
...
flags.DEFINE_float(
    ...
    1.0,
    "Weight that determines the answer"
    "loss importance in comparison to that of the cell loss. Only used if"
...
flags.DEFINE_float(
    "huber_loss_delta",
    25.0,
    "The delta used to calculate the huber loss."
...
flags.DEFINE_float(
    "answer_loss_cutoff",
    100.0,
    "If set, model weights will not be"
...
flags.DEFINE_float(
    "temperature",
    1.0,
    "Used to scale cell logits. A lower temperature"
...
flags.DEFINE_float(
    "agg_temperature",
    1.0,
    "Used to scale agg logits. A lower temperature"
    "produces probabilities that looks like stepwise functions. A value of 1.0"
    "means no scaling.",
)
...
flags.DEFINE_bool(
    ...
    False,
    "Whether gumbel-softmax distribution should " "be used for cell selection.",
)

flags.DEFINE_bool(
    "use_gumbel_for_agg",
    False,
    "Whether gumbel-softmax distribution should "
...
flags.DEFINE_float(
    "grad_clipping",
    None,
    "If not None, gradients greater in absolute value" "than this number are clipped.",
)

flags.DEFINE_string(
    "eval_interactions_file", None, "The file that contains interactions protos."
)
flags.DEFINE_string(
    "predict_interactions_file", None, "The file that contains interactions protos."
)
...
flags.DEFINE_string(
    "table_pruning_config_file", None, "To specify the config for table pruning."
)
...
flags.DEFINE_integer(
    "restrict_attention_bucket_size",
    0,
    "For sparse attention modes, further "
    "restrict attention to consecutive buckets of uniform size.",
)

flags.DEFINE_integer(
    "restrict_attention_header_size",
    None,
    "For sparse attention modes, size "
...
flags.DEFINE_float(
    ...
    0.5,
    "For sparse attention modes, "
    "proportion of heads that should focus on rows vs columns.",
)

flags.DEFINE_bool(
    "cell_cross_entropy", False, "Whether to use cross entropy for cell selection loss."
)

flags.DEFINE_bool(
    "cell_cross_entropy_hard_em",
    False,
...
flags.DEFINE_bool(
    "compute_weighted_denotation_accuracy",
    False,
...
flags.DEFINE_bool(
    "allow_empty_column_selection", False, "If false, disallow selecting no column."
)

flags.DEFINE_bool(
    "mask_examples_without_labels", False, "If false, mask examples without answers."
)
...
def _predict_and_export_metrics(
    ...
    if FLAGS.do_sequence_prediction:
        ...
        output_predict_file_sequence = os.path.join(
            FLAGS.model_dir, base_name + "_results_sequence_{}.tsv"
        ).format(current_step)
        prediction_utils.write_predictions(
            result_sequence,
...
def main(_):
    ...
    tapas_config = tapas_classifier_model.TapasClassifierConfig(
        ...
        restrict_attention_mode=(
            attention_utils.RestrictAttentionMode(FLAGS.restrict_attention_mode)
        ),
        ...
        init_cell_selection_weights_to_zero=FLAGS.init_cell_selection_weights_to_zero,
    ...
    if tapas_config.classification_label_weight:
        if any(x < 0 for x in tapas_config.classification_label_weight.values()):
            raise ValueError(
                "Label weights cannot be negative in input: "
                f"{tapas_config.classification_label_weight}."
            )
        if any(
            x < 0 or x >= tapas_config.num_classification_labels
            for x in tapas_config.classification_label_weight.keys()
        ):
            raise ValueError(
                "Invalid label in label weights for input: "
                f"{tapas_config.classification_label_weight}."
            )
    ...
    if FLAGS.do_train:
        ...
        if FLAGS.table_pruning_config_file:
            table_pruning_path = os.path.join(
                FLAGS.model_dir, "table_pruning_config.textproto"
            )
            tf.io.gfile.copy(
        ...
        tapas_config.to_json_file(os.path.join(FLAGS.model_dir, "tapas_config.json"))
    ...
    if FLAGS.do_eval:
        ...
        for _, checkpoint in experiment_utils.iterate_checkpoints(
            ...
            marker_file_prefix=os.path.join(estimator.model_dir, f"eval_{eval_name}"),

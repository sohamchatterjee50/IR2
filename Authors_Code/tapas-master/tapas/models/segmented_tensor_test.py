...
class SegmentedTensorTest(tf.test.TestCase):
    ...
    def test_product_index(self):
        ...
        with self.session() as sess:
            ...
            self.assertEqual(
                sess.run(row_index.num_segments), sess.run(row_index_proj.num_segments)
            )
    ...
    def test_flatten(self):
        ...
        with self.session() as sess:
            ...
            self.assertEqual(sess.run(batched_index_flat.num_segments), np.prod(shape))
            self.assertAllEqual(
    ...
    def test_gather_vectorized(self):
        ...
        with self.session() as sess:
            self.assertAllEqual(sess.run(result), [[[1, 2], [3, 4]], [[7, 8], [5, 6]]])

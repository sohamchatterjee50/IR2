...
def _calculate_aggregation_logits(
...
def _single_column_cell_selection_loss(
    token_logits, column_logits, label_ids, cell_index, col_index, cell_mask
):
...
def _get_classification_outputs(
    ...
    with tf.variable_scope("loss"):
        total_loss = 0.0
        is_supervised = not do_model_aggregation or not config.use_answer_as_supervision
...
def _calculate_eval_metrics_fn(
    ...
    token_correct = tf.logical_or(
        tf.equal(label_ids, predictions), tf.logical_not(tf.cast(input_mask, tf.bool))
    )
    # <bool>[batch size]
        ...
        metrics.update(
            {
                ...
                "eval_mean_per_class_classification_accuracy": mean_per_class_accuracy_cls,
            }
        )

    if logits_aggregation is not None:
        # <int32>[batch size]
        predictions_agg = tf.argmax(logits_aggregation, axis=-1, output_type=tf.int32)
        accuracy_agg = tf.metrics.accuracy(
            labels=aggregation_function_id, predictions=predictions_agg
        )
        # <bool>[batch size]
        per_sequence_agg_accuracy = tf.equal(aggregation_function_id, predictions_agg)
        # Whether cells and aggregation function predictions are both correct.
        per_sequence_joint_accuracy = tf.logical_and(
            per_sequence_agg_accuracy, per_sequence_accuracy
        )
    ...
    def model_fn(features, labels, mode, params):
        ...
        if config.reset_output_cls:
            tvars = [
                ...
                for tvar in tvars
                if (
                    "output_weights_cls" not in tvar.name
        ...
        if mode == tf_estimator.ModeKeys.TRAIN:
            ...
            if do_model_aggregation:
                predictions.update(
                    {
                        "gold_aggr": features["aggregation_function_id"],
                        "pred_aggr": tf.argmax(
                            outputs.logits_aggregation,
                            axis=-1,
            ...
            if do_model_classification:
                ...
                if config.num_classification_labels == 2:
                    predictions.update(
                        {
                            "logits_cls": outputs.logits_cls[:, 1]

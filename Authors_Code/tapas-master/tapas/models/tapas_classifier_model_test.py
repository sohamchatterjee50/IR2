...
class TapasClassifierModelTest(parameterized.TestCase, tf.test.TestCase):
    ...
    def _create_estimator(self, params):
        ...
        tapas_config = tapas_classifier_model.TapasClassifierConfig(
            ...
            restrict_attention_mode=params.get(
                "restrict_attention_mode", attention_utils.RestrictAttentionMode.FULL
            ),
            restrict_attention_bucket_size=2,
            restrict_attention_header_size=4,
            cell_cross_entropy=params.get("cell_cross_entropy", False),
            cell_cross_entropy_hard_em=params.get("cell_cross_entropy_hard_em", False),
        )
        ...
        estimator = tf_estimator.tpu.TPUEstimator(
            params={
                ...
                "fail_if_missing_variables_in_checkpoint": params.get(
    ...
    def test_build_model_train_and_evaluate(
        ...
        if load_checkpoint:
            ...
            if attention in [attention_utils.RestrictAttentionMode.TABLE_ATTENTION]:
    ...
    def test_build_model_predict(self, use_answer_as_supervision, select_one_column):
    ...
    def _build_pruning_unsupervised_config(
        self, use_columns, bert_config_file, add_classification_loss
    ):
        if use_columns:
    ...
    def _print_pruning_metrics(
        self, eval_metrics, display_columns=None, display_pruning_loss=True
    ):
        metrics = [
            "eval_sequence_accuracy",
            "eval_mean_label",
    ...
    @parameterized.named_parameters(
        ("unsupervised_columns_without_pruning_loss", "unsupervised_columns", False),
        ("unsupervised_columns_with_pruning_loss", "unsupervised_columns", True),
        ("unsupervised_tokens_without_pruning_loss", "unsupervised_tokens", False),
        ("unsupervised_tokens_with_pruning_loss", "unsupervised_tokens", True),
    )
    def test_build_model_train_and_evaluate_pruning(
        self, test_file, add_classification_loss
    ):
        """Tests that we can train, save, load and evaluate the model."""

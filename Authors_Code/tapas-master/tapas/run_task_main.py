...
flags.DEFINE_string(
    "input_dir", None, "Directory where original shared task data is read from."
)
...
flags.DEFINE_bool(
    "test_mode", False, "Cut some corners to test the pipeline end-to-end."
)
...
flags.DEFINE_bool(
    "loop_predict", True, "Loop predictions as new checkpoints appear while training"
)
...
flags.DEFINE_bool(
    "prune_columns", False, "Use word overlap heuristics to keep most relevant columns."
)
...
flags.DEFINE_boolean(
    "use_document_title", False, "Use table title to encode the input."
)
...
flags.DEFINE_boolean(
    "drop_rows_to_fit",
    False,
    "Drop last rows if table does not fit within max sequence " "length.",
)

flags.DEFINE_string("table_pruning_config_file", None, "Table pruning config file.")
...
def _train_and_predict(
    ...
    elif task in [tasks.Task.WTQ, tasks.Task.WIKISQL, tasks.Task.WIKISQL_SUPERVISED]:
    ...
    tapas_config = tapas_classifier_model.TapasClassifierConfig(
        ...
        span_prediction=tapas_classifier_model.SpanPredictionMode(
            hparams.get(
                "span_prediction", tapas_classifier_model.SpanPredictionMode.NONE
            )
        ),
    ...
    if mode == Mode.CREATE_DATA:
        ...
        if task != tasks.Task.NQ_RETRIEVAL:
            ...
            task_utils.create_interactions(
                task, FLAGS.input_dir, output_dir, token_selector
            )

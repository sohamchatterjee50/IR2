...
def _calc_weighted_denotation_accuracy(
    tables_file, examples, denotation_errors_path, predictions_file_name, add_weights
):
...
def _write_synthetic_table(test_tmpdir):
    ...
    data = [
        ...
        ["Melbourne", "13.15", "Fitzroy", "8.11", "MCG", '"14,931"', "14 May 1927"],
...
def _write_tables_dict(headers=None, data=None):
    ...
    output_tables_file = tempfile.mktemp(prefix="output_tables_file_", dir=test_tmpdir)
...
class CalcMetricsTest(parameterized.TestCase):
    ...
    def test_read_data_examples(self):
        ...
        self.assertEqual(
            examples["dev-2-0_0"].gold_cell_coo,
    ...
    def test_read_predictions(self):
        ...
        self.assertEqual(
            examples["dev-2-0_0"].gold_agg_function, calc_metrics_utils._Answer.COUNT
        )
        self.assertEqual(
            examples["dev-2-0_0"].pred_agg_function, calc_metrics_utils._Answer.NONE
        )
    ...
    def test_calc_structure_metrics(self):
        ...
        self.assertEqual(denotation_errors.iloc[0, 2], calc_metrics_utils._Answer.NONE)
        self.assertEqual(denotation_errors.iloc[0, 3], calc_metrics_utils._Answer.NONE)
    ...
    def test_denotation_accuracy(self):
        ...
        denotation_errors = pd.read_csv(
            os.path.join(
                test_tmpdir, "denotation_examples_{}".format(predictions_file_name)
            ),
            sep="\t",
        )
        self.assertEqual(denotation_errors.iloc[0, 1], "dev-0-0_0")
    ...
    def test_read_predictions_without_pred_aggr(self):
        ...
        examples = {
            "dev-0-0_0": calc_metrics_utils.Example(

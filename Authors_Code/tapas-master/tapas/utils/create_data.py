...
_KeyInteractionTable = Tuple[
    Text, Tuple[interaction_pb2.Interaction, Optional[interaction_pb2.Table]]
]
...
def _merge_random_interactions(key_interactions):
    ...
    for key, interaction in interactions:
        ...
        if prev_table.table_id != interaction.table.table_id:
            ...
            beam.metrics.Metrics.counter(
                _NS, "Interactions without random interaction"
            ).inc()
...
def build_pretraining_pipeline(
    ...
    def _pipeline(root):
        ...
        interactions = (
            ...
            | "CheckTableSize"
            >> beam.FlatMap(
                pretrain_utils.check_tale_size_fn, min_num_rows, min_num_columns
            )
        )
        if add_numeric_values:
            interactions = interactions | "AddNumericValues" >> beam.Map(
                pretrain_utils.add_numeric_values_fn
            )
        else:
            interactions = interactions | "AddText" >> beam.Map(_add_text_fn)
        interactions = interactions | "Duplicate" >> beam.FlatMap(
            pretrain_utils.duplicate_fn, dupe_factor
        )
        ...
        if add_random_table:
            ...
            interactions = interactions | "PairWithNone" >> beam.FlatMap(
                pretrain_utils.pair_with_none_fn
            )

        examples = (
            interactions
            | f"ToTensorflowExample_{config.max_seq_length}"
            >> beam.ParDo(pretrain_utils.ToTensorflowExample(config))
        )
...
def read_tables(root, input_file, name):
    inputs = _read_inputs(root, input_file, name, proto_message=interaction_pb2.Table)
    return inputs | f"to_interaction {name}" >> beam.Map(_to_interaction_fn)


def read_only_tables(root, input_file, name):
    return _read_inputs(root, input_file, name, proto_message=interaction_pb2.Table)
...
def build_classifier_pipeline(
    ...
    def _pipeline(root):
        ...
        for input_file, output_file in zip(input_files, output_files):
            ...
            if use_fake_tables:
                interactions = interactions | f"InsertFakeTable_{name}" >> beam.Map(
                    insert_fake_table_fn
                )
            examples = (
                interactions
                | f"CheckTableId_{name}"
                >> beam.FlatMap(pretrain_utils.check_table_id_fn)
                | f"AddNumericValues_{name}"
                >> beam.Map(pretrain_utils.add_numeric_values_fn)
                | f"ToClassifierTensorflowExample_{name}"
                >> beam.ParDo(
            ...
            pretrain_utils.write_proto_outputs(
                output_file, f"WriteExamples_{name}", examples, tf.train.Example
            )
...
class ToRetrievalTensorflowExample(beam.DoFn):
    ...
    def process(self, element):
        ...
        if not interaction.questions:
            ...
            beam.metrics.Metrics.counter(
                _NS, "Fake Questions added for table only example"
            ).inc()
    ...
    def _pipeline(root):
        ...
        for input_file, output_file in zip(input_files, output_files):
            ...
            examples = (
                ...
                | f"AddNumericValues_{name}"
                >> beam.Map(pretrain_utils.add_numeric_values_fn)
                | f"ToRetrievalTensorflowExample_{name}"
                >> beam.ParDo(

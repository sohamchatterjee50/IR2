...
class CreatePretrainingDataTest(parameterized.TestCase):
    ...
    def test_end_to_end_multiple_interactions(self, mock_read):
        with tf.gfile.Open(
            os.path.join(self._test_dir, "interaction_01.pbtxt")
        ) as input_file:
            interaction = text_format.ParseLines(
                input_file, interaction_pb2.Interaction()
            )
...
class CreateClassifierDataTest(parameterized.TestCase):
    ...
    @mock.patch.object(beam.io, "ReadFromTFRecord")
    def test_gracefully_handle_big_examples(
        self, max_seq_length, max_column_id, max_row_id, expected_counters, mock_read
    ):

        with tf.gfile.Open(
            os.path.join(self._test_dir, "interaction_02.pbtxt")
        ) as input_file:
            interaction = text_format.ParseLines(
                input_file, interaction_pb2.Interaction()
            )
        ...
        if max_seq_length is None and max_column_id is None and max_row_id is None:
            ...
            with tf.gfile.Open(
                os.path.join(self._test_dir, "tf_example_02.pbtxt")
            ) as input_file:
                expected_example = text_format.ParseLines(
                    input_file, tf.train.Example()
                )
            with tf.gfile.Open(
                os.path.join(self._test_dir, "tf_example_02_conv.pbtxt")
            ) as input_file:
    ...
    def test_candidates(self, mock_read):
        ...
        with tf.gfile.Open(
            os.path.join(self._test_dir, "interaction_03.pbtxt")
        ) as input_file:
            interaction = text_format.ParseLines(
                input_file, interaction_pb2.Interaction()
            )
        ...
        with tf.gfile.Open(
            os.path.join(self._test_dir, "tf_example_03.pbtxt")
        ) as input_file:
...
class CreateRetrievalDataTest(parameterized.TestCase):
    ...
    def test_end_to_end(self, input_format, impl, mock_read):
        ...
        with tf.gfile.Open(
            os.path.join(self._test_dir, "retrieval_interaction.pbtxt")
        ) as input_file:
            interaction = text_format.ParseLines(
                input_file, interaction_pb2.Interaction()
            )

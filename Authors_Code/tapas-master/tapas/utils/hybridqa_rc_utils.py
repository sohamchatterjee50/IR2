...
def get_predictions(prediction_file):
    ...
    fn_map = {
        "logits_cls": float,
        "position": int,
        "answer_coordinates": lambda x: list(prediction_utils.parse_coordinates(x)),
...
def get_table_dimensions(table):
    ...
    if isinstance(table, interaction_pb2.Table):
        ...
        try:
            ...
            raise ValueError(
                f"Invalid table json. Key not present: '{e.args[0]}'"
            ) from e
...
class HybridQaRcConfig:
    ...
    def hybridqa_prediction_filepaths(self):
        ...
        return {
            (split, 0): prediction_file_path
            for split, prediction_file_path in self.hybridqa_prediction_filepath.items()
        }
...
def _parse_answer_cell(cell, text, links, descriptions, *, url_unquote=True):
    ...
    for link in collections.OrderedDict.fromkeys(links):
        ...
        cell_annotation.identifier = urllib.parse.unquote(link) if url_unquote else link
...
def _parse_answer_table(
    ...
    answer_table = _init_answer_table(
        json_dict["uid"], json_dict["title"], json_dict["url"]
    )
...
def _create_answer_table(
    ...
    for coords in answer_coordinates:
        ...
        _parse_answer_cell(
            row.cells.add(), original_cell.text, links, descriptions, url_unquote=False
        )
    return answer_table


def generate_sample_space(strategy, n_rows, n_cols, true_coordinates):
...
def sample_noisy_coordinates(
    n_rows, n_cols, answer_coordinates, strategy, n_samples, random_state
):
    ...
    *,
    ...
    if "answer-text" in example:
        ...
        if single_cell_examples:
            answer_tables = _parse_answer_tables(
                table_json, descriptions, selected_coordinates
            )
            ...
            answer_table = _parse_answer_table(
                table_json, descriptions, selected_coordinates
            )
...
def remove_unreferred_annotation_descriptions(interaction):
...
def _create_eval_answer_interaction(interaction, answer_table, i_table=0):
...
def _create_eval_answer_interactions(
    interaction, predicted_coordinates, *, single_cell_examples=False
):
    ...
    if interaction.questions:
        ...
        if single_cell_examples:
            for i_table, coordinate in enumerate(predicted_coordinates):
                answer_table = _create_answer_table(
                    interaction.table, descriptions, [coordinate]
                )
            ...
            answer_table = _create_answer_table(
                interaction.table, descriptions, predicted_coordinates
            )
            new_interaction = _create_eval_answer_interaction(
...
def _convert_split(
    ...
    logging.info(
        "Found %d/%d ambiguous examples in %s",
        multi_answer_count,
        len(examples),
        split.value,
    )
...
def create_interactions_from_hybridqa_predictions(
    ...
    for (split, run_id), predictions_file in splits_dict.items():
        ...
        for p in get_predictions(predictions_file):
            ...
            answer_coordinates[example_id] = ranked_cells[
                : config.num_predictions_to_keep
            ]
        original_interactions_file = os.path.join(
            interaction_dirs[run_id], f"{split.lower()}.tfrecord"
        )
...
def convert(
    ...
    json_tables = {
        key: json_map
        for key, json_map in hybridqa_utils.read_json_directory(table_dir).items()
    }
    ...
    return {
        split.value: _convert_split(split, main_path, json_tables, descriptions, config)
        for split in _Split
    }

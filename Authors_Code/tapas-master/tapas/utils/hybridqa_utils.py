...
_TRANSLATION_TABLE = str.maketrans(string.punctuation, " " * len(string.punctuation))
...
def get_cell_selection_metrics(
    ...
    if not questions:
        return {}
    cell_selection_metrics = dict(eval_cell_selection(questions, predictions_file))[
        AnswerType.ALL
    ]
...
def eval_cell_selection(
    ...
    for answer_type in AnswerType:
        if total[answer_type]:
            recall_at_k = {
                f"recall_at_{k}": (
                    (total_correct_at_k[k][answer_type] / total[answer_type])
                    if has_probabilities
                    else None
                )
                for k in _RECALL_KS
            ...
            yield answer_type, CellSelectionMetrics(
                ...
                precision=(
                    (sum_precision[answer_type] / total_non_empty[answer_type])
                    if total_non_empty[answer_type]
                    else None
                ),
                **recall_at_k,
            )
...
def find_answer_coordinates(answer_text, table, desc_map):
...
def find_dataset_coordinates(example_json_or_question):
...
def _parse_interaction(
    ...
    if "answer-text" in example:
        true_coordinates, table_only_coordinates, matched_identifiers = (
            find_answer_coordinates(example["answer-text"], table, desc_map)
        )
...
def _convert_split(split, main_path, table_dict, descriptions_dict):

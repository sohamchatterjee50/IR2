...
_Proto = Union[interaction_pb2.Interaction, interaction_pb2.Table, tf.train.Example]
...
def check_table_id_fn(key_interaction):
    ...
    if (
        interaction.id
        and interaction.table.table_id
        and all(bool(q.id) for q in interaction.questions)
    ):
...
def check_tale_size_fn(key_interaction, min_num_rows, min_num_columns):
...
def duplicate_fn(key_interaction, dupe_factor):
    key, interaction = key_interaction
...
class ToTensorflowExample(beam.DoFn):
    ...
    def start_bundle(self):
        self._converter = tf_example_utils.ToPretrainingTensorflowExample(self._config)
    ...
    def process(self, element):
        ...
        seed = tf_example_utils.fingerprint(
            "%s_%d_%d" % (key, self._config.random_seed, self._config.max_seq_length)
        )
...
def write_proto_outputs(output_file, name, data, proto_message):
    ...
    if output_file.endswith((".txtpb.gz", ".txtpb")):
        ...
        _ = (
            data
            | "DropKey_%s" % name >> beam.Values()
            | "WriteTFRecordsExamples_%s" % name
...
def split_by_table_id_and_write(
    ...
    train, test = examples | "Partition" >> beam.Partition(partition_fn, 2, num_splits)
...
def read_inputs(root, input_file, name, proto_message):
    ...
    if input_file.endswith((".txtpb.gz", ".txtpb")):
        return (
            root
            | f"Read {name}" >> beam.io.textio.ReadFromText(input_file)

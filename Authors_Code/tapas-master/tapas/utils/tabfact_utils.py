...
_AGGREGATION_WORDS = {"only", "there", "total", "count", "average", "sum", "amount"}
...
def get_interesting_features(accuracy, heardoom, num_features):
...
def read_questions(input_dir):
    collected_data = {}
    for i in [1, 2]:
        input_file = os.path.join(input_dir, f"collected_data/r{i}_training_all.json")
...
def convert(
    ...
    return {
        split: _convert_data(questions, os.path.join(input_dir, "data", file), tables)

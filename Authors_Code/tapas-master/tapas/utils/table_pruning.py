...
class Scores:
    ...
    gather_op: Optional[
        Callable[[Dict[Text, tf.Tensor], tf.Tensor], Dict[Text, tf.Tensor]]
    ] = None
...
class TablePruningSelector:
    ...
    def compute_loss(
        self, required_columns, column_scores, column_score_mask, token_scores
    ):
...
class ModelPruningSelector(TablePruningSelector):
    ...
    def compute_loss(
        ...
        return self._model.compute_loss(
            required_columns, column_scores, column_score_mask, token_scores
        )
    ...
    def _get_index_gather(self, scores, input_mask):
        """Computes the indexes of gather_op given a scores tensor."""
        # <float32>[batch_size, seq_len]
        scores_mask = self._mask_top_k.apply_hard_selection(scores=scores) * tf.cast(
    ...
    def compute_scores(self, mode, features):
        ...
        hard_selection = self.apply_hard_selection(mode=mode, scores=stats.token_scores)
        input_mask = features["input_mask"]
    ...
    def _compute_columns_stats(self, column_scores, column_ids):
...
class OnesTablePruning(ModelPruningSelector):
    ...
    def _select_columns(self, mode, features):
        del mode
        input_shape = modeling.get_shape_list(features["input_ids"], expected_rank=2)
...
class TapasPruningSelector(ModelPruningSelector):
    ...
    def __init__(self, config, max_num_columns, max_num_rows, max_num_tokens):
        ...
        self._bert_config = modeling.BertConfig.from_json_file(config.bert_config_file)
...
def create_selector(
    ...
    with tf.gfile.Open(table_pruning_config_file) as input_file:
        # ParseLines
        config = text_format.ParseLines(
            input_file, table_pruning_pb2.TablePruningModel()
        )
...
class Unsupervised(LossSelector):
    ...
    def __init__(self, config, max_num_tokens):
        super(Unsupervised, self).__init__(config=config, max_num_tokens=max_num_tokens)
...
def get_mask_columns_scores(max_num_columns, column_ids, scores):
    ...
    """
    # <float32>[batch_size, max_num_columns]
    ranges = tf.ones_like(scores) * tf.cast(tf.range(max_num_columns), dtype=tf.float32)
    # <float32>[batch_size, max_num_columns]
...
def get_token_scores_from_column_scores(
    ...
    return segmented_tensor.gather(
        index=col_index, values=padded_column_scores
    ) * tf.cast(input_mask, dtype=tf.float32)
...
def get_table_pruning_loss(
    ...
    if (
        table_selector.should_add_classification_loss()
        and table_selector.get_sequence_output() is not None
    ):

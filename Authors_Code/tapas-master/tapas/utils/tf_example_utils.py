...
def _get_all_answer_ids_from_coordinates(
    ...
    if missing_count:
        beam_metrics.Metrics.counter(
            _NS, _get_buckets(missing_count, buckets, "Missing answers")
        ).inc()
    if found_answers:
        beam_metrics.Metrics.counter(
            _NS, _get_buckets(len(found_answers), buckets, "Found answers")
        ).inc()
...
def _get_all_answer_ids(
    ...
    def _to_coordinates(
        ...
        return [
            (coords.column_index, coords.row_index)
...
def _get_answer_ids(column_ids, row_ids, question):
    """Maps answer coordinates to token indexes."""
    answer_ids, missing_count = _get_all_answer_ids(column_ids, row_ids, [question])
...
def _add_entity_descriptions_to_table(
    ...
    logging.log_first_n(
        logging.INFO,
        "%s selected entity annotations for %s:  %s",
        100,
        question.id,
        question.text,
        search_results,
    ...
    beam_metrics.Metrics.counter(
        _NS, _get_buckets(sentences_kept, buckets, "Descriptions kept")
    ).inc()
    beam_metrics.Metrics.counter(
        _NS, _get_buckets(sentences_discarded, buckets, "Descriptions discarded")
    ).inc()
            ...
            if annotated_cell_ext in cell.Extensions:
                for annotation in cell.Extensions[annotated_cell_ext].annotations:
                    ...
                    if filtered_sentences:
                        if use_entity_title:
                            annotation_name = _get_annotation_name(
                                annotation.identifier
                            )
                            cell.text += (
                                f" ( {annotation_name} : {filtered_sentences} )"
                            )
                        else:
...
class ToTensorflowExampleBase:
        ...
        for (column_index, row_index), relations in cell_indices_to_relations.items():
            ...
            for relation in relations:
                ...
                relation_set_index += 2 ** (
                    relation.value - constants.Relation.EQ.value
                )
            beam_metrics.Metrics.counter(
                _NS, "Relation Set Index: %d" % relation_set_index
            ).inc()
            for cell_token_index in _get_cell_token_indexes(
                column_ids, row_ids, column_index, row_index
            ):
            ...
            for col_index in range(len(table.columns)):
                for row_index in range(len(table.rows)):
                    ...
                    if float_value == float("inf"):
                        beam_metrics.Metrics.counter(
                            _NS, "cell with numeric value of infinite"
                        ).inc()
                        continue
    ...
    def _add_numeric_values_scale(self, table, token_ids_dict, features):
        ...
        features["numeric_values_scale"] = create_float_feature(numeric_values_scale)
...
class ToPretrainingTensorflowExample(ToTensorflowExampleBase):
    ...
    def _to_example(self, table, instance):
        ...
        masked_lm_ids = self._tokenizer.convert_tokens_to_ids(instance.masked_lm_labels)
    ...
    def convert(
        ...
        if table is None:
            ...
            if not question_tokens or len(question_tokens) < self._min_question_length:
                beam_metrics.Metrics.counter(
                    _NS,
            ...
            beam_metrics.Metrics.counter(
                _NS,
                _get_buckets(
                    len(question_tokens), self._question_buckets, "Question Length"
                ),
            ).inc()
            ...
            serialized_example = self._serialize(
                question_tokens, tokenized_table, num_columns, num_rows, num_tokens
            )
    ...
    def _create_masked_lm_predictions(
        self, interaction, tokens, column_ids, row_ids, rng
    ):
    ...
    def _get_question_tokens(self, interaction, rng):
    ...
    def _get_table_sizes(self, token_budget, table, rng):
        ...
        beam_metrics.Metrics.counter(
            _NS, _get_buckets(num_columns * num_rows, buckets, "Trimmed Table Size")
        ).inc()
        ...
        beam_metrics.Metrics.counter(
            _NS,
            _get_buckets(real_num_columns * real_num_rows, buckets, "Real Table Size"),
        ).inc()
        beam_metrics.Metrics.counter(
            _NS, _get_buckets(num_columns, buckets, "Column Sizes")
        ).inc()
        beam_metrics.Metrics.counter(
            _NS, _get_buckets(num_rows, buckets, "Row Sizes")
        ).inc()
        beam_metrics.Metrics.counter(
            _NS, _get_buckets(num_tokens, buckets, "Table Token Sizes")
        ).inc()
...
class ToClassifierTensorflowExample(ToTrimmedTensorflowExample):
    ...
    def __init__(self, config):
        ...
        self._entity_descriptions_sentence_limit = (
            config.entity_descriptions_sentence_limit
        )
    ...
    def convert(self, interaction, index):
        ...
        if not interaction.questions[index].answer.is_valid:
            beam_metrics.Metrics.counter(
                _NS, "Conversion skipped (answer not valid)"
            ).inc()
        ...
        if (
            self._expand_entity_descriptions
            and annotation_descriptions_ext in interaction.Extensions
        ):
        ...
        if table_selection_ext in question.Extensions:
            ...
            if table_selection.selected_tokens:
                ...
                tokenized_table.selected_tokens = [
                    t
                    for t in tokenized_table.selected_tokens
        ...
        answer = (
            ...
            if question.answer.HasField("float_value")
            else _NAN
        )
        ...
        if self._add_aggregation_candidates:
            ...
            for index, candidate in enumerate(candidates):
                ...
                for row in candidate.rows:
                    token_indexes += _get_cell_token_indexes(
                        column_ids, row_ids, candidate.column, row
                    )
            ...
            if num_initial_candidates > 0:
                ...
                if num_final_candidates != num_initial_candidates:
                    beam_metrics.Metrics.counter(
                        _NS, "Candidates: Dropped candidates"
                    ).inc()

...
def _get_sqa_table_id(wtq_table_id):
    ...
    return "table_csv/" + wtq_table_id[4:].replace("/", "-").replace("-csv", "")
...
def _export_table(table, output_dir, sqa_table_id):
...
def _convert_data(
    ...
    with _get_reader(_get_sqa_file_path(input_dir, file_name)) as file_in:
        ...
        df.to_csv(
            tf.io.gfile.GFile(output_file, "w"), sep="\t", index=False, encoding="utf-8"
        )

...
import os, json, tqdm, hydra, collections
import tensorflow._api.v2.compat.v1 as tf
from argparse import Namespace
from omegaconf import DictConfig
from tapas.protos import interaction_pb2, retriever_info_pb2
from tapas.utils import text_utils, bm25_utils, prediction_utils, nq_preprocess_utils
...
def _try_to_set_answer(
    ...
    coordinates_to_answers, other_answers = [], []
    for answers in answer_groups:
        ...
        for answer in answers:
            ...
            coords = list(
                nq_preprocess_utils.find_answer(
                    table,
                    answer,
...
def _get_neural_nearest_neighbors(file_pattern):
...
@hydra.main(version_base=None, config_path="configs", config_name="end2end")
def main(cfg: DictConfig):
    ...
    interaction_files = []
    for filename in tf.io.gfile.listdir(args.input_dir):

        interaction_files.append(os.path.join(args.input_dir, filename))
        ...
        tables.update(
            {
                ...
                for table in bm25_utils.iterate_tables(args.table_file)
            }
        )
    ...
    if args.index_files_pattern:
        neighbors = _get_neural_nearest_neighbors(args.index_files_pattern)
        retrieve_fn = lambda question: neighbors.get(question.id, [])

    else:
        index = bm25_utils.create_bm25_index(
            tables=tables.values(),
            title_multiplicator=args.title_multiplicator,
    ...
    for interaction_file in interaction_files:
        ...
        for interaction in interactions:
            ...
            example_id, _ = nq_preprocess_utils.parse_interaction_id(interaction.id)
        ...
        with tf.io.TFRecordWriter(output) as writer:
            ...
            with tqdm.tqdm(
                ...
                for nr, example in enumerate(pbar):
                    ...
                    candidate_hits = scored_hits[: args.max_rank]
                    ...
                    if is_train or args.oracle_retrieval:

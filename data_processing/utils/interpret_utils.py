...
def _clip_candidates(rng, candidates):
...
def _find_count_candidates(rng, table, float_value):
...
def get_correct_powerset(indices, values, is_correct_fn):
    ...
    for subset in itertools.chain.from_iterable(
        itertools.combinations(indices, r) for r in range(1, len(indices) + 1)
    ):
...
def _find_numeric_cell_combinations(rng, table, is_correct_fn, agg_function):
    ...
    for column in range(num_columns):
        ...
        for row, cells in enumerate(table.rows):
            ...
            if (
                cell.numeric_value.HasField("float_value")
                and cell.numeric_value.float_value != float("INF")
                and not math.isnan(cell.numeric_value.float_value)
            ):
...
def _find_sum_candidates(rng, table, float_value):

    def _is_correct(indices, values):
        return (
            math.fabs(sum(values[i] for i in indices) - float_value) < _FLOAT_TOLERANCE
        )

    return _find_numeric_cell_combinations(rng, table, _is_correct, _Answer.SUM)


def _find_average_candidates(rng, table, float_value):

    def _is_correct(indices, values):
        length = float(len(indices))
        return (
            math.fabs(sum(values[i] for i in indices) / length - float_value)
            < _FLOAT_TOLERANCE
        )

    return _find_numeric_cell_combinations(rng, table, _is_correct, _Answer.AVERAGE)


def find_candidates(rng, table, question):
    ...
    """
    if (
        not question.answer.HasField("float_value")
        or math.isnan(question.answer.float_value)
        or question.answer.float_value == float("INF")
    ):
        return []
    ...
    candidates += _find_average_candidates(rng, table, question.answer.float_value)

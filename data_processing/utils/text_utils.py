...
def wtq_normalize(x):
    ...
    This normalization function is taken and modified from WikiTableQuestions github, hence the
    ...
    # Define a mapping of characters to their replacements
    char_map = {
        "‘": "'",
        "’": "'",
        "´": "'",
    ...
    x = pattern.sub(lambda m: char_map[m.group(0)], x)
    ...
    x = re.sub(r"\s+", " ", x, flags=re.U).strip().lower()
    x = re.sub("<[^<]+?>", "", x)
    x = x.replace("\n", " ")
...
def create_interaction_id(example_id, ith_table=None, suffix=0):
...
def find_all_substrings(needle, haystack):

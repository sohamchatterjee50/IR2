## The Main BERT Model Definition and its Functions
...
def _get_absolute_position_embeddings(
    full_position_embeddings, seq_length, width, num_dims
):
...
def _get_relative_position_embeddings(
    ...
    first_position_per_segment = segmented_tensor.reduce_min(
        batched_position, full_index
    )[0]
    first_position = segmented_tensor.gather(first_position_per_segment, full_index)
...
def embedding_postprocessor(
    ...
    if extra_embeddings is not None:
        flat_extra_embeddings = tf.reshape(
            extra_embeddings, [batch_size * seq_length, -1]
        )
...
def attention_layer(
    ...
    if attention_mask is not None and to_proj_length is None:
        ...
        attention_mask_float = tf.where(
            attention_mask_float > 0.0,
        ...
        adder = tf.where(
            tf.is_finite(adder), adder, tf.zeros_like(adder, dtype=tf.float32) - 10000.0
        )
    ...
    if relative_relation_ids is not None:
        relative_attention_scores = compute_relative_attention_scores(
            query_layer,
...
def reshape_from_matrix(output_tensor, orig_shape_list):
    ...
    orig_dims, width = orig_shape_list[0:-1], output_shape[-1]

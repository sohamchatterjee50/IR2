...
def create_optimizer(
    ...
    if gradient_accumulation_steps > 1:
        optimizer = GradientAccumulationOptimizer(
            optimizer, steps=gradient_accumulation_steps, grad_clipping=grad_clipping
        )
...
class GradientAccumulationOptimizer(tf.train.Optimizer):
    ...
    def __init__(
        self, opt, steps, grad_clipping=None, name="GradientAccumulationOptimizer"
    ):
        ...
        for grad, param in grads_and_vars:
            ...
            if self.grad_clipping is not None:
                grad = tf.clip_by_value(
                    grad, -1 * self.grad_clipping, self.grad_clipping
                )

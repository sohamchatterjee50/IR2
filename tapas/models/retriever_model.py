...
def _get_assignment_map_from_checkpoint(
    tvars, init_checkpoint, init_from_single_encoder
):
    ...
    def _fill_assignments(
        assignment_map, initialized_variable_names, var_name_from_init, var_name
    ):
    ...
    if init_from_single_encoder:
        ...
        for x in init_vars:
            ...
            _fill_assignments(
                assignment_map_additional_encoder,
                initialized_variable_names,
                name,
                name_additional,
            )
...
def model_fn_builder(config: RetrieverConfig):
    ...
    def model_fn(features, labels, mode, params):
        ...
        empty_table_features = tf.zeros_like(features["question_input_mask"])[
            :, : config.max_query_length
        ]
        features_query = {
            "input_ids": features["question_input_ids"][:, : config.max_query_length],
            "input_mask": features["question_input_mask"][:, : config.max_query_length],
            "segment_ids": empty_table_features,
            "column_ids": empty_table_features,
            "row_ids": empty_table_features,
            "prev_label_ids": empty_table_features,
            "column_ranks": empty_table_features,
            "inv_column_ranks": empty_table_features,
            "numeric_relations": empty_table_features,
        }
        ...
        if config.down_projection_dim > 0:
            ...
            table_rep = _get_type_representation(
                table_hidden_representation, table_projection
            )
        ...
        table_id_hash = tf.reshape(tf.transpose(features["table_id_hash"]), shape=[-1])
        # <int64>[batch_size, 1]
        ...
        if init_checkpoint:
            (assignment_maps, initialized_variable_names) = (
...
def input_fn(
    name: Text,
    file_patterns: Iterable[Text],
    data_format: Text,
    is_training: bool,
    max_seq_length: int,
    include_id: bool,
    compression_type: Text,
    use_mined_negatives: bool,
    params,
):

...
import os, re, csv, enum, json, functools, collections
...
def _iterate_jsonl_file(input_dir, suffix):
...
def _convert_tables(input_dir, output_dir):
    ...
    for _, _, table in _iterate_jsonl_file(input_dir=input_dir, suffix=".tables.jsonl"):
        with tf.io.gfile.GFile(
            os.path.join(table_dir, table["id"] + ".csv"), "wb"
        ) as csvfile:
...
def _convert_questions(input_dir, output_dir):
    ...
    with tf.io.gfile.GFile(os.path.join(output_dir, "train.tsv"), "wb") as train_file:
        with tf.io.gfile.GFile(os.path.join(output_dir, "test.tsv"), "wb") as test_file:
            with tf.io.gfile.GFile(
                os.path.join(output_dir, "dev.tsv"), "wb"
            ) as dev_file:
                writers = {
                    "train": csv.DictWriter(
                        train_file, delimiter="\t", fieldnames=header
                    ),
                    "test": csv.DictWriter(
                        test_file, delimiter="\t", fieldnames=header
                    ),
                    "dev": csv.DictWriter(dev_file, delimiter="\t", fieldnames=header),
                }
                ...
                for tag, index, example in _iterate_jsonl_file(
                    ...
                    try:
                        ...
                        float_answer = _get_float_answer(
                            table, answer_coordinates, aggregation_op
                        )
                    ...
                    writers[tag].writerow(
                        {
                            ...
                            "table_file": f"table_csv/{example['table_id']}.csv",
                            "answer_coordinates": [
                                str(ac) for ac in answer_coordinates
                            ],
                            "answer_text": _get_answer_text(
                                table, answer_coordinates, float_answer
                            ),
                            "aggregation": _get_aggregation_name(aggregation_op),
                            "float_answer": (

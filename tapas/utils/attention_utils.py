## Helper Methods for Restricting the Transformer Model Attention

import math, enum, functools, dataclasses
import tensorflow._api.v2.compat.v1 as tf
from tapas.utils.constants import _INFINITY
...
def _matches_token_type_id(tensor):
    ...
    """
    return tf.math.equal(tf.expand_dims(tensor, axis=1), tf.expand_dims(tensor, axis=2))


def compute_sparse_attention_mask(segment_ids, column_ids, row_ids, input_mask, **_):
    ...
    return tf.cast(
        ...
        & (
            _matches_token_type_id(column_ids)
            | _matches_token_type_id(row_ids)
...
def _compute_cross_bucket_attention_mask(
    bucket_size, header_size, token_type_id, input_mask
):
    ...
    bucket_id = _compute_bucket_id(bucket_size, header_size, token_type_id, input_mask)
    ...
    return (
        tf.math.less_equal(bucket_diff, 1)
...
def compute_headwise_sparse_attention_mask(
    ...
    **_,
    ...
    to_or_from_segument_zero = tf.expand_dims(segment_zero, axis=2) | tf.expand_dims(
        segment_zero, axis=1
    )
...
def _create_bucketed_tensor(
    ...
    with tf.variable_scope(name):
        ...
        def _rotate(t, offset):
            return tf.concat(
                [t[:, :, offset:, Ellipsis], t[:, :, :offset, Ellipsis]],
                axis=2,
...
def _compute_bucketed_attention_mask(from_view, to_view, ids, attend_always):
    ...
    mask = (ids.expand_to(to_view) < _INFINITY) & (
        ...
        | attend_always.expand_from(from_view)
        | attend_always.expand_to(to_view)
    )
...
def create_bucketed_attention_layer(
        ...
        with tf.variable_scope(name):
            ...
            for tensor, indices in zip(tensors, type_ids_argsorted):
                gathered_tensors.append(
                    tensor
                    if indices is None
                    else tf.gather(tensor, indices, batch_dims=1)
                )
    ...
    if sort_after_projection:
        ...
        attend_always_repeated = _gather_and_repeat(
            [input_header], "attend_always_gather"
        )
    ...
    attend_always = _create_bucketed_tensor(
        attend_always_repeated, "attend_always", **args
    )
    head_full_mask = _compute_bucketed_attention_mask(
        "head", "full", ids, attend_always
    )
    tail_head_mask = _compute_bucketed_attention_mask(
        "tail", "head", ids, attend_always
    )
...
def _bucketed_attention_layer(
    ...
    attention_scores_from_head = _attention_scores(
        "head", "full", head_full_mask, queries, keys
    )
    ...
    context_layer = tf.concat(
        [context_layer_from_head, context_layer_from_tail], axis=2
    )

    # Reorder applying the inverse permutation.
    context_layer = tf.gather(context_layer, type_ranks, batch_dims=2, name="reorder")

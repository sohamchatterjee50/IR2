...
from typing import Any, List, Optional, Set, Text, Tuple
...
def read_data_examples_from_interactions(interactions_path):
...
def example_from_question(
    ...
    float_value = (
        question.answer.float_value if question.answer.HasField("float_value") else None
    )
    class_index = (
        question.answer.class_index if question.answer.HasField("class_index") else None
    )
    ex = Example(
        ex_id,
        question_text,
...
def calc_structure_metrics(examples, denotation_errors_path=None):
    ...
    if denotation_errors_path:
        ...
        with tf.io.gfile.GFile(
            os.path.join(denotation_errors_path, "structured_examples.tsv"), "w"
        ) as f:
            frame.to_csv(f, sep="\t")
...
def execute(aggregation_type, cell_coos, table):
    ...
    if aggregation_type == _Answer.NONE:
        ...
        if not values and (
            aggregation_type == _Answer.AVERAGE or aggregation_type == _Answer.SUM
        ):
...
def _get_gold_denotation_result(example):
    ...
    if example.float_answer is None:
        ...
        denotation, values = [(example.float_answer)], []
...
def calc_weighted_denotation_accuracy(
...
def calc_denotation_accuracy(examples, denotation_errors_path, predictions_file_name):
    """Calculates the denotation accuracy."""
    return calc_weighted_denotation_accuracy(
        examples, denotation_errors_path, predictions_file_name, add_weights=False
    )["denotation_accuracy"]

...
import os, re, ast, json, string, dataclasses, collections
import numpy as np
import tensorflow._api.v2.compat.v1 as tf
from absl import logging
from typing import Optional, Text
from tapas.protos import interaction_pb2
from tapas.utils import text_utils, prediction_utils
...
class DeTokenizer:
    ...
    def _detokenize_text(
        ...
        start_token, end_token = tokens[start_token_index], tokens[end_token_index]
...
def _is_valid_answer(token_ids):
    ...
    # Token id 1 corresponds to the [EMPTY] token.
...
def _get_reconstructed_answer_text(
    ...
    if "answers" in prediction:
        token_answers = prediction_utils.token_answers_from_text(prediction["answers"])
...
def _get_answer_texts(interactions):
    """Returns answer text."""
    answer_texts = set()
    for interaction in interactions:
...
def _compute_f1(a_gold, a_pred):
    ...
    gold_toks, pred_toks = _tokenize(a_gold), _tokenize(a_pred)
...
def is_table_answer(question_id, references):
    return False if references is None else question_id in references["table"]


def is_passage_answer(question_id, references):
    return False if references is None else question_id in references["passage"]


def _evaluate_retrieval_e2e(vocab_file, interaction_list, predictions, references=None):
    ...
    for candidates in interactions.values():
        ...
        if metrics["exact"] != 1 and num_errors_logged < 100:
            num_errors_logged += 1
            logging.info(
                "question: '%s' references: %s prediction: '%s'",
                _get_question(candidates),
                reference_answer_texts,
                best_answer_text,
            )
...
def get_hybridqa_references(references_path):
...
def evaluate_retrieval_e2e(
    ...
    if references_file is not None:
        references = get_hybridqa_references(references_file)
    ...
    return _evaluate_retrieval_e2e(vocab_file, interactions, predictions, references)

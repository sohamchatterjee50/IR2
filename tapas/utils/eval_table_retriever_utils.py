...
def _get_gold_ids_in_global_indices(queries, tables):
    ...
    valid_query_indices = [
        i for i, query in enumerate(queries) if all(table_id in table_id_to_index for table_id in query.table_ids)
    ]
...
def _filter_queries_with_matching_tables(queries, tables):
    ...
    filtered_queries = [
        query for query in queries if any(table_id in table_ids for table_id in query.table_ids)
    ]

...
import os, re, bs4, enum, html, json, math, hashlib, dataclasses, collections
import urllib.parse
import numpy as np
import pandas as pd
...
# The _MAX_INT here is larger than the one more generally used
_MAX_INT = 2**64 - 1
...
def _str(value):
    ...
    beam.metrics.Metrics.counter(_NS, f"Dataframe cell types: {type(value)}").inc()
...
def _is_non_standard_table(table):
    ...
    values = table.values
    is_too_many_rows_eq = (
        sum([len(set(row)) == 1 for row in values]) > (values.shape[0] / 2) - 1
    )
...
def _parse_table(table_html):
    ...
    try:
        ...
        return None
    is_table_infobox = (
        "infobox" in table.attrs["class"] if "class" in table.attrs else False
    )
    if is_table_infobox:
...
def table_contains_all_answers(table, question):
...
def _create_interaction(
    ...
    interaction.id = get_interaction_id(example_id, main_answer_index)
...
def _extract_interaction(
    ...
    for (table_start, table_end), table in span_to_table.items():
        ...
        for answer_index, (spans, texts) in enumerate(zip(annotations_spans, answers)):
            matches = [
                (start, end)
...
def _get_spans(html_bytes, annotations):
    ...
    for annot in annotations:
        ...
        if not answers:
            beam.metrics.Metrics.counter(
                _NS, "Removing annotation without short answers"
            ).inc()
            continue

        elif len(answers) != 1:
            beam.metrics.Metrics.counter(
                _NS, "Annotations with multiple short answers"
            ).inc()
...
def write_interactions_to_files(
    ...
    def tfrecord(name):
        return tf.io.TFRecordWriter(os.path.join(interaction_dir, f"{name}.tfrecord"))

    with tfrecord("train") as train_writer, tfrecord("dev") as dev_writer, tfrecord(
        "test"
    ) as test_writer:
        for interaction in interactions:

            stats["total"] += 1
            if not table_contains_all_answers(
                interaction.table, interaction.questions[0]
            ):

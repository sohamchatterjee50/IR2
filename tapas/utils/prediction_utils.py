...
import os, ast, csv, copy, json, dataclasses, collections
import numpy as np
import pandas as pd
...
def read_classifier_dataset(
    ...
    dataset = classifier_model.input_fn(
        name="predict",
...
def _to_token_answer(
    ...
    cell_begin_token_index = list(zip(row_ids, column_ids)).index((row_id, column_id))
...
def _get_token_answers(
    ...
    if span_indexes is not None and span_logits is not None:
        best_logit, best_span = max(
            zip(span_logits, span_indexes.tolist()),
        )
        logging.log_every_n(
            logging.INFO,
            "best_span: %s, score: %s",
    ...
    answers, answer_probs = [], []
    answer_begin_index, answer_end_index = None, None
    for i, prob in get_cell_token_probs(prediction):
...
def _write_prediction(
    ...
    max_width, max_height = prediction["column_ids"].max(), prediction["row_ids"].max()
    ...
    if output_token_probabilities:
        token_probabilities = [
            (
                int(prediction["column_ids"][i]) - 1,
                int(prediction["row_ids"][i]) - 1,
                float(prob),
            )
            for i, prob in get_cell_token_probs(prediction)
        ]
...
def write_json(predictions, output_predict_file):

## Defines the Column Selection Methods

import enum, queue, collections, dataclasses
import apache_beam as beam
from typing import Set, Optional
from nltk.stem.porter import PorterStemmer
from tapas.utils.constants import _NS
from tapas.utils import text_utils, tf_example_utils
from tapas.protos import interaction_pb2, table_selection_pb2
...
def _get_question_cost(
    ...
    tokens = tokenizer.tokenize(text_utils.normalize_for_match(question.original_text))
...
def from_selected_columns_to_selected_tokens(interaction, selected_columns, tokenizer):
...
def _get_question_column_similarity(column_tokens, question_tokens, epsilon=0.0):
...
class MaxTokenSelector(TokenSelector):
    ...
    def _get_column_tokens(self, interaction, column_index):
        column_tokens = []
    ...
    def _get_row_tokens(self, interaction, row_index):
        row_tokens = []
        for column_index in range(len(interaction.table.columns)):
    ...
    def _get_header_tokens(self, interaction):
        row_tokens = []
    ...
    def _get_row_similarity_scores(self, interaction, question_tokens):
    ...
    def _update_priority_queue_from_previous_answer(
        ...
        if len(interaction.questions) > 1:
            ...
            if index > 0:
                answer_coords = interaction.questions[
                    index - 1
                ].answer.answer_coordinates
                if selection_type == SelectionType.COLUMN:
                    ...
                    answer_coords = [
                        (c.row_index, c.column_index) for c in answer_coords
                    ]
    ...
    def _get_columns_from_priority_queue(
        ...
        if num_columns_skipped == 0:
            ...
            beam.metrics.Metrics.counter(_NS, "Columns removed").inc(
                num_columns_skipped
            )
            ...
            if answer_column_index not in selected_columns:
                ...
                if not question.answer.HasField("float_value"):
                    beam.metrics.Metrics.counter(
                        _NS, "Unambiguous columns removed"
                    ).inc()
    ...
    def _get_tokens_from_priority_queue(
        ...
        if num_tokens_skipped == 0:
            ...
            beam.metrics.Metrics.counter(_NS, "Tokens removed").inc(num_tokens_skipped)

        if num_cells_skipped == 0:
            beam.metrics.Metrics.counter(
                _NS, "Selected at least one token of every cell"
            ).inc()

        else:
            beam.metrics.Metrics.counter(_NS, "Cells removed (no token selected)").inc(
                num_cells_skipped
            )

        if num_columns_skipped == 0:
            beam.metrics.Metrics.counter(
                _NS, "Selected at least one token of every column"
            ).inc()

        else:
            beam.metrics.Metrics.counter(
                _NS, "Columns removed (no token selected)"
        ...
        if scored_zero > 0:
            beam.metrics.Metrics.counter(_NS, "Answer cell is scored 0").inc(
                scored_zero
            )
...
class HeuristicExactMatchTokenSelector(MaxTokenSelector):
    ...
    def _select_columns(
        ...
        return self._get_columns_from_priority_queue(
            columns_queue, interaction, question
        )
    ...
    def _select_cells(
        ...
        return self._get_tokens_from_priority_queue(tokens_queue, interaction, question)


class SelectAllTokensFn(TokenSelector):
    """Extracts all the columns."""

    def select_tokens(self, interaction, question):

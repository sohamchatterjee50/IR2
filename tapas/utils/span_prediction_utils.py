## Utilities for Answer Span Prediction
...
def _get_span_logits(
    ...
    batch_size, seq_length, embedding_dim = shape[0], shape[1], shape[2]
    spans, num_spans, flat_spans_2d = _get_spans(
        seq_length, batch_size, max_span_length
    )
...
def get_boundary_logits(
    ...
    loss = _get_span_loss(start_logits, start_ids) + _get_span_loss(end_logits, end_ids)
...
def get_span_logits_by_mode(
    mode, embeddings, label_ids, column_ids, row_ids, max_span_length
):
